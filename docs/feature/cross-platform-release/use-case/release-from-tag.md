...
3. Target jobs first build the pinned linux-x64-glibc and darwin-arm64 engines. One Ubuntu 24.04 runner builds and typechecks, packs once, verifies the digest, and installs that exact npm artifact containing both target-native engines with isolated `init`/`check` and genuine HawDB miss/hit/clear/recall checks with Rust and HawDB helpers absent from PATH.
