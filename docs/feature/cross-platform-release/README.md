...
The supported release is Linux x86_64/aarch64 and Apple Silicon macOS 14/15 on local APFS, with Node.js 24.15+, Git and `flock`. Homebrew supplies Node and the keg-only `util-linux` flock on macOS. Nix remains Linux-only. Windows, Intel macOS, HFS and network filesystems are outside the current compatibility claim.
