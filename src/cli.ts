...
root.pipe(Command.withSubcommands([init,recover,...(['feature','use-case','research','design','roadmap'] as const).map(docsGroup),author,memory,issue,test,cache,check,trace,review]),Command.run({version:'0.1.0'}),Effect.catch(cause=>Effect.sync(()=>{
