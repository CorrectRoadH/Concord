...
const root = Command.make('concord').pipe(Command.withDescription('Connect product contracts, test annotations, and engineering memory. Agent guidance: concord --skill [topic].'), Command.withSharedFlags({
...
const init = Command.make('init', { testRoot: many('test-root'), docsOnly: Flag.boolean('docs-only').pipe(Flag.withDefault(false)), runnerConfig: optional('runner-config') }, args => withRepo((repo, s) => sync(() => {
  ...
  return initialize(repo, s.dryRun, { testRoots: args.docsOnly ? [] : args.testRoot.length ? args.testRoot : undefined, runner });
}), { initialize: true })).pipe(Command.withDescription('Initialize templates guidance and configuration without overwriting files. --docs-only disables test discovery; --test-root repeats; --runner-config reads a runner JSON object.'));
...
root.pipe(Command.withSubcommands([Command.make('repo').pipe(Command.withDescription('Run this worktree repository profile with its original contracts and formal evidence.')),init,recover,...(['feature','use-case','research','design','roadmap','engineering'] as const).map(docsGroup),author,memory,issue,test,cache,check,trace,review,templates,diagnose]),Command.run({version:'0.3.0'}),Effect.catch(cause=>Effect.sync(()=>{
