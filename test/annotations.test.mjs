...
function write(root, relative, contents) { const path = join(root, relative); mkdirSync(join(path, '..'), { recursive: true }); writeFileSync(path, contents); }
function scan(root, options) { const repo = new LocalRepository(root); try { return scanAnnotations(repo, options); } finally { repo.close(); } }

test('indexes real runner imports and safely rebuilds SQLite projections', () => {
  const root = consumer();
  try {
    write(root, 'test/example.test.mjs', `import spec from 'node:test';
// @concord-case parser-case
// @concord-contract feature/parser
// @concord-regression memory/parser-bug
spec('parses comments', () => {});
// @concord-case retired-case
// @concord-contract feature/parser
// @concord-status retired
spec.skip('old parser', () => {});
function test() {}
test('ordinary helper', () => {});
const text = '// @concord-case fake';
`);
    const cold = scan(root);
    assert.equal(cold.cache.status, 'miss'); assert.equal(cold.cases.length, 2);
    assert.deepEqual(cold.findings, []);
    assert.equal(cold.cases.find(item => item.id === 'retired-case')?.skipped, true);
    assert.equal(cold.cases.find(item => item.id === 'retired-case')?.status, 'retired');
...
// @concord-case parser-case
// @concord-contract feature/new-parser
test('parses comments v2', { skip: true }, () => {});
`);
    const modified = scan(root); assert.equal(modified.cache.status, 'miss'); assert.equal(modified.cases[0]?.contract, 'feature/new-parser');
...
test('reports unsupported, duplicate, orphan annotations and a corrupt cache without trusting it', () => {
  const root = consumer();
  try {
    write(root, 'test/findings.test.ts', `import * as nodeTest from 'node:test';
// @concord-case duplicate-case
// @concord-contract feature/a
nodeTest.test('first', () => {});
// @concord-case duplicate-case
// @concord-contract feature/b
nodeTest.test('second', () => {});
// @concord-case orphan-case
const value = 1;
nodeTest.test.each([])('dynamic', () => {});
const fake = ` + '`// @concord-case template-fake`' + `;
`);
    const first = scan(root);
    assert.ok(first.findings.some(item => item.code === 'DuplicateCaseId'));
    ...
    const repo = new LocalRepository(root); let path;
    ...
    const cache = new DatabaseSync(path); try {
      ...
      cache.prepare('UPDATE annotation_cache SET payload = ? WHERE cache_key = ?').run(JSON.stringify({ cases: [], findings: [], files: [], digest: 'sha256:not-a-real-digest' }), row.cache_key);
    } finally { cache.close(); }
