...
import { getGitDiff, getGitStatus } from '../dist/git-view.js';

// @concord-case inspect-staged-unstaged-and-new-files
...
// @concord-case git-test-baseline-identities
// @concord-contract docs/feature/web-workbench/use-case/use-web-workbench.md
test('Git test baseline uses committed AST declarations and keeps IDs across file moves', async () => {
  ...
  try {
    ...
    const source = "import test from 'node:test';\n// @concord-case existing-case\n// @concord-contract docs/feature/example/README.md\ntest('Existing', () => {});\n";
    writeFileSync(join(root, 'old.test.ts'), source);
    writeFileSync(join(root, 'fake.ts'), 'const example = `// @concord-case fake-case`;\n');
    git('add', '.'); git('commit', '-qm', 'baseline');
    git('mv', 'old.test.ts', 'renamed.test.ts');
    writeFileSync(join(root, 'renamed.test.ts'), source + "// @concord-case new-case\n// @concord-contract docs/feature/example/README.md\ntest('New', () => {});\n");
    git('add', 'renamed.test.ts');
    const status = await Effect.runPromise(getGitStatus(root, true));
    assert.deepEqual(status.baselineCaseIds, ['existing-case']);
  } finally { rmSync(root, { recursive: true, force: true }); }
});
