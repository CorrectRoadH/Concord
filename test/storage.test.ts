...
// @concord-case storage-project-outlines-recover-exact-paths
...
test('project outlines preview without writes and recover using exact document paths', () => Effect.runPromise(Effect.sync(() => {
 ...
 try {
  ...
  writeFileSync(join(root, '.git/concord/journal.json'), JSON.stringify({ format: 'concord.journal', root, privateDir: join(root, '.git/concord'), projectId, operation: 'outlines', phase: 'prepared', directories: [], scope: { kind: 'documents' }, changes }));
...
test('dry-run creates no private state; init conflicts and symlinks preserve existing content',()=>Effect.runPromise(Effect.sync(()=>{
 const root=fixture();try {
  ...
  repo=new LocalRepository(root,{initialize:true});assert.throws(()=>initialize(repo,false,{testRoots:['node_modules']}),{code:'InvalidSourcePath'});repo.close();assert.equal(existsSync(join(root,'concord.json')),false);
...
// @concord-case storage-unified-journal-preflights-and-rejects-legacy-formats
// @concord-contract docs/feature/local-sdlc/use-case/recover-local-state.md
test('unified journal recovers interrupted init, preflights every target, and preserves unknown formats',()=>Effect.runPromise(Effect.sync(()=>{
 const root=fixture();let repo: LocalRepository | undefined;try {
  const config=JSON.stringify({format:'concord.project/v1',projectId:'init-recovery',testRoots:[],runner:{kind:'node-test',sourceFiles:[],timeoutMs:60000}},null,2)+'\n';
  ...
  writeFileSync(journalPath,JSON.stringify({format:'concord.journal',root,privateDir:join(root,'.git','concord'),projectId:'init-recovery',operation:'init',phase:'prepared',directories:['docs'],scope:{kind:'documents'},changes:[
   {path:'concord.json',before:null,after:config,beforeDigest:null,afterDigest:digest(config),mode:420},
   {path:'docs/concord.md',before:null,after:guide,beforeDigest:null,afterDigest:digest(guide),mode:420},
  ]})+'\n');
  repo=new LocalRepository(root,{recover:true});assert.equal(repo.recover().status,'rolled-back');assert.equal(existsSync(join(root,'concord.json')),false);assert.equal(existsSync(join(root,'docs/concord.md')),false);repo.close();repo=undefined;
  const initial=new LocalRepository(root,{initialize:true});initialize(initial);initial.close();
  writeFileSync(join(root,'memory/a.md'),'before-a');writeFileSync(join(root,'memory/b.md'),'before-b');const marker=readFileSync(join(root,'concord.json'),'utf8');
  const batch={format:'concord.journal',root,privateDir:join(root,'.git','concord'),projectId:JSON.parse(marker).projectId,operation:'batch',phase:'prepared',directories:[],scope:{kind:'documents'},changes:[
  ...
  writeFileSync(join(root,'memory/a.md'),'after-a');writeFileSync(join(root,'memory/b.md'),'external');writeFileSync(journalPath,JSON.stringify(batch)+'\n');repo=new LocalRepository(root,{recover:true});assert.throws(()=>repo!.recover(),{code:'RecoveryConflict'});assert.equal(readFileSync(join(root,'memory/a.md'),'utf8'),'after-a');assert.equal(existsSync(journalPath),true);
  rmSync(journalPath);writeFileSync(journalPath,'{"format":"concord.journal/v2"}\n');assert.throws(()=>repo!.recover(),{code:'InvalidData'});assert.equal(existsSync(journalPath),true);
 } finally {repo?.close();rmSync(root,{recursive:true,force:true});}
})));
