...
代码关系每次从当前 Markdown 重算，并核对文件集合与摘要。声明解析可命中 Git-private 的可重建投影（普通 checkout 通常为 `.git/concord/cache.hawdb`，用 `concord cache status` 查询实际位置）；字节摘要变化、损坏或 `cache clear` 后回源。清缓存不能修复源码错误。
