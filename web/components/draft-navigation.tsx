...
export function useDraftNavigation<Value = string>(change: (value: Value) => void) {
  const { dirty, flushAutoSave } = useWorkspace();
  const [pending, setPending] = useState<Value | null>(null);
  const request = (value: Value) => { if (!dirty) { change(value); return; } void flushAutoSave().then(saved => { if (saved) change(value); else setPending(value); }); };
