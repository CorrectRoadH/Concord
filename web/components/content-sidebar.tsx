...
function DirectoryLinks({ items, root, query, onNavigate }: { items: readonly ContentSidebarItem[]; root: string; query: string; onNavigate(): void }) {
  ...
  const render = (directory: Directory): ReactNode => <>
    ...
    {[...directory.files].sort((a, b) => a.id.split('/').at(-1)!.localeCompare(b.id.split('/').at(-1)!)).map(item => <li key={item.id}><Link to={item.href} title={item.id} aria-current={item.active ? 'page' : undefined} onClick={onNavigate}>{item.icon}<span>{item.id.split('/').at(-1)}</span></Link></li>)}
  </>;
  return render(tree);
}
...
export function ContentSidebar({ model, actions }: { model: ContentSidebarModel; actions?: ReactNode }) {
  ...
  const content = <nav aria-label="内容导航" className="document-navigation__content">
    ...
    {model.groups.map((group, index) => {
      ...
      <ul aria-label={group.label}>{group.treeRoot ? <DirectoryLinks items={items} root={group.treeRoot} query={query} onNavigate={() => setOpen(false)} /> : items.map(item => <li key={item.id}>
