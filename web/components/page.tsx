...
export function Empty({ title = '这里还没有内容', children, kind = 'empty' }: { title?: string; children?: ReactNode; kind?: 'empty' | 'filtered' | 'missing' }) {
  return <div className="empty" data-kind={kind}>{kind === 'filtered' ? <SearchX size={20} /> : kind === 'missing' ? <FileQuestion size={20} /> : null}<strong>{title}</strong>{children && <p>{children}</p>}</div>;
}
